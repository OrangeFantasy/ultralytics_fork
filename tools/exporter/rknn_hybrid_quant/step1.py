...
if __name__ == '__main__':
    ...
    # Pre-process config
    print('--> Config model')
    rknn.config(mean_values=[0, 0, 0], std_values=[255, 255, 255], target_platform='rk3588', optimization_level=3)
    print('done')
    ...
    print('--> Loading model')
    ret = rknn.load_onnx("runs/_qta_test_/train3/weights/native/model_fp_simplified.onnx", input_size_list=[[1, 3, 448, 768]])
    if ret != 0:
    ...
    import os
    dataset_txt = os.path.dirname(__file__) + '/dataset.txt'
