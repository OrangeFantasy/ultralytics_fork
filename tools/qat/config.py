...
def get_ball_sports_qat_config(args):
    def _forward_function(self, x: List[torch.Tensor]):
        ...
        x0 = [self.op_concat([box0[l], cls0[l], kpts[l], angles[l]], dim=1) for l in range(self.nl)]
        x1 = [self.op_concat([box1[l], cls1[l]], dim=1) for l in range(self.nl)]
        return (*x, *x0, *x1)
    ...
    def _forward_qat(self, preds):
        ...
        box0, cls0, kpts, angles, box1, cls1 = map(list, zip(*[
            (
        ...
        angles = torch.cat([angles[i].view(bs, self.n_angles, -1) for i in range(self.nl)], dim=-1)
...
def get_sports_qrcode_qat_config(args):
    def _forward_function(self, x: List[torch.Tensor]):
        box0 = [self.cv2[0][i](x[i]) for i in range(self.nl)]
        box1 = [self.cv2[1][i](x[i]) for i in range(self.nl)]
